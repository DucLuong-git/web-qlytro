...
module.exports = (io) => {
  ...
  chatNamespace.on('connection', (socket) => {
    ...
    // ------------------------------------------
    // 2.5 EVENT: rename_room - Đổi tên nhóm chat
    // ------------------------------------------
    socket.on('rename_room', async (data, callback) => {
      try {
        ...
        if (!isSystemAdmin && !participant) {
          if (callback) callback({ success: false, error: 'Bạn không có quyền.' });
          return;
        }

        const updatedRoom = await ChatRoom.findByIdAndUpdate(roomId, { name }, { new: true });
        
        // Broadcast cho những người khác trong phòng cập nhật tên mới
        chatNamespace.to(roomId).emit('room_renamed', updatedRoom);
